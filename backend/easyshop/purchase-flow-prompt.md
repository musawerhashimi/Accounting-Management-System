...
now i want you to handle for me the purchase flow, such that
in purchase items along with the required fields for purchase item like unit_cost and others, you send either variant_id when you wanna buy a product that exists in the system and if you send the expire date too, the backend creates a batch and assign to the purchase item, or instead of variant_id user sends the required fields to make a product and it includes like name, category_id, unit, ... with variants(always a default variant is required) which includes image, is_default, (variant_name for the default variant is the name of the product and for other variants it is required).
cost_price cost_currency, selling_price selling_currency, also fields like expire date (to make it batch-tracked) and others needed
also for default variant these four fields are required and for others not
and for this item backend creates a product record, one or more variant records (always default is required to make a new product record), if batch tracked a batch record is created, and then assign the variant_id, and batch_id(if created) to the purchas_item.
also in handling different status of the purchase like draft, pending, ordered or approved, partially_recieved, recieved...
making stock changes like effecting the inventory, stock movements table, in pay, effecting payments, transactions, cash_drawers and others
and also proper update handling.
i rely on your logic of accounting
