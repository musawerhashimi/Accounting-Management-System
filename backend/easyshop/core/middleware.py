...
from .models import Tenant, ActivityLog
from .threads import set_current_tenant, clear_current_tenant
...
class TenantMiddleware(MiddlewareMixin):
    """
    Middleware to set tenant context based on domain or header.
    """
    
    def process_request(self, request):
        """
        Set tenant context from domain or X-Tenant-ID header.
        """
        tenant = None
        
        # Method 1: Get tenant from X-Tenant-ID header (for API clients)
        ...
        # Method 2: Get tenant from domain (for web clients)
        if not tenant:
            host = request.get_host().split(':')[0]  # Remove port if present
            # host = request.get_host().split('/')[1]  # Get the Subdomain
            path = request.path.strip('/').split('/')[0]
            try:
                tenant = Tenant.objects.get(domain=path, status='active')
            except Tenant.DoesNotExist:
